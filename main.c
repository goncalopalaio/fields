...
void influence_ray_grid_mouse() {	
	...
	for (int x = 0; x < RAY_SIDE_GRID * RAY_SIDE_GRID; ++x){
		...
		if (intersection_res != FALSE){
			...
			current->direction.x *= 0.9;
			current->direction.y *= 0;
			current->direction.z *= 0.9;

			M_ADD3(current->direction, current->direction, current->position);
		}
		idx+=6;
	}
}		
...
void build_grid() {
	float scale_between_points = 0.3;
	...
	for (int x = 0; x < RAY_SIDE_GRID; ++x){
		for (int y = 0; y < RAY_SIDE_GRID; ++y){
			...
			ray_grid_points_vertices[idx + 3] = x_pos + 0;
...
void update_humans() {
	float accumulation_factor = 0.01;
	float distance_threshold = 0.01;
	int n_points_in_grid = RAY_SIDE_GRID * RAY_SIDE_GRID;
	for (int i = 0; i < NUMBER_HUMAN_POINTS; ++i){closest_grid_point_idx[i] = -1;}
	...
	for (int h = 0; h < NUMBER_HUMAN_POINTS; ++h){
		...
		for (int i = 0; i < n_points_in_grid; ++i) {
			...
			if (dist < distance_threshold) {
				...
				human_points_vertices[idx_humans + 0] += (curr_grid_point->direction.x - curr_grid_point->position.x) * accumulation_factor;
				human_points_vertices[idx_humans + 1] += (curr_grid_point->direction.y - curr_grid_point->position.y) * accumulation_factor;
				human_points_vertices[idx_humans + 2] += (curr_grid_point->direction.z - curr_grid_point->position.z) * accumulation_factor;
				break;
			}
...
int main(int argc, char const *argv[]){
	...
	int number_grid_points = RAY_SIDE_GRID * RAY_SIDE_GRID;
	ray_grid_points_vertices = (float*) malloc(number_grid_points * 2 * 3 * sizeof(float));
	ray_grid_points_colors = (float*) malloc(number_grid_points * 2 * 3 * sizeof(float));	
	...
	while(!glfwWindowShouldClose(window)) {
		...
		float cam_speed = 20;
		set_vec(&camera.position, 10 ,10, 0);
		...
		glEnableVertexAttribArray(0);
		glEnableVertexAttribArray(1);
		glDrawArrays(GL_LINES, 0, RAY_SIDE_GRID * RAY_SIDE_GRID * 2);
		glUseProgram(0);
